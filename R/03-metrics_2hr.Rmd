# Metrics for 2hr Bins

We want the following metrics for times the device was deployed:

* **Total proportion of time wet:** proportion of seconds wet across all 2-hour bins

* **Total number of state switches (wet/dry transitions) per 10mins:** number of times the device switched between wet (non-zero) and dry (zero) states across the deployment time (without downsampling).

* **Cumulative dry and wet times:** total time (in seconds) the device recorded as completely dry (wets == 0) or wet (wets > 0)

* **Longest continuous periods of dry or wet states throughout the whole deployment period:** longest sequence of consecutive dry/wet 10-minute intervals within the entire deployment period


I start with visualizing proportions/switches for Baccalieu Island (2017-2018) using both types of downsampled data (taking the average or sum of wets).
...
Add the colony and deployment_period values from metrics_md to `deg_props_2hrs` and `deg_switches`
...
```{r}
plotly_switch <- ggplotly(plot_metrics(bin_metrics, y_var = "switches", y_label = "Number of Wet-Dry Switches (2-hour bins)", plot_title = "Switches Between Wet-Dry States Across Deployment Period"))
...
Aggregate bins to DAYS for visualizing daily proportions, switches and wet/dry times
```{r}
...
So when we look by day, for these birds we see a good trend, between Dec 2017 and May 2018 there is a high proportion of wet periods. The total number of times the device switches from wet to dry reduces during the periods where there is a lot of wets, seems to make sense.
...
DEC-16 NOTE: this needs to be fixed somethign wrong with the POSTIXct dates for datetime_end
DEC-17 NOTE: I can't figure it out but it doesn't seem to be causing any issues in date parsing
```{r}
...
longest_prds
```
**NOTE:** longest_wet is a vlue of how many bins. So if longest_wet = 764, that means that the bird's device recorded wetness for at least part of each 2-hour period over a continuous span of 1528 hours (764 bins). **It does not imply that the bird was continuously wet throughout the entire 2-hour bins during this period** since there may have been fluctuations between wet and dry states within each bin.
...
plot_longest_prds <- ggplot(longest_prds, aes(y = bird_id)) +
  ...
  # Wet at least once in 2-hour bin (dotted black line)
...
As a consequence, the lightblue line here showing the longest period when wet was greater than 0 looks like it spans across the whole time. There were definitely periods within those 2 hour bins when wets = 0 for longer though.\
