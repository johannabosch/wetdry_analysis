...
plotly_bacc_2017_heatmap_prop <- plot_ly(
...
calc_longest_prds <- function(file_path) {
  ...
  calc_prds <- function(condition) {
    ...
    end_x <- cumsum(rle_wet$lengths)[ #cumul. sum of bin lengths for run end index
      which.max(rle_wet$lengths[rle_wet$values])] #index = longest period
    
    start_x <- end_x - longest_wet +1 #just subtract # of bins in wet prd
    ...
    list(
      ...
      duration = longest_run * bin_time, #duration is the longest period we found times the bin time
      )
    
  #calculate the periods 
    longest_wet <- calc_prds(deg_data$wets>0)
    longest_max_wet <- calc_prds(deg_data$wets==20)
  ...
  return(data.frame(
    ...
    longest_entirely_wet = longest_max_wet$duration,
    longest_dry = longest_dry$duration,
    datetime_start_wet = longest_wet$start,
    datetime_end_wet = longest_wet$end,
    datetime_start_maxwet = longest_max_wet$start,
    datetime_end_maxwet = longest_max_wet$end,
...
plotly_bacc_2017_longest_prds <- ggplotly(plot_bacc_2017_longest_prds) %>%
  layout(margin = list(l =50, r=50, b=100, t=50),
         xaxis = list(tickangle=45))
```
Looks like dry periods are too short to visualise here
Plot that next to our heatmap of proportions 

```{r}


subplot(plotly_bacc_2017_heatmap_prop, plot_bacc_2017_longest_prds,
        nrows = 2, shareX = TRUE, shareY = FALSE)
```
...
```{r}
bacc_2017_longest_wet_prds_long <- bacc_2017_longest_wet_prds %>%
  pivot_longer(cols = starts_with("datetime"), 
               names_to = "period_type", 
               values_to = "datetime") %>%
  filter(!is.na(datetime)) %>%
  mutate(period_type = case_when(
    str_detect(period_type, "start_wet") ~ "Wet (at least once)",
    str_detect(period_type, "start_maxwet") ~ "Entirely Wet",
    str_detect(period_type, "start_dry") ~ "Dry"
  ))

# Heatmap
ggplot(bacc_2017_longest_wet_prds_long, aes(x = datetime, y = bird_id, fill = period_type)) +
  geom_tile(alpha = 0.8) +
  scale_fill_manual(values = c("Wet (at least once)" = "darkblue",
                               "Entirely Wet" = "lightblue",
                               "Dry" = "darkgrey")) +
  labs(title = "Heatmap of Longest Periods",
       x = "Time", y = "Bird ID",
       fill = "Period Type") +
  theme_minimal() +
  theme(axis.text.y = element_text(size = 10),
