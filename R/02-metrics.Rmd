...
calc_longest_wet_prds <- function(file_path) {
  ...
  #when avg wets is greater than 0
  rle_wet <- rle(deg_data$wets > 0)
  longest_wet = max(rle_wet$lengths[rle_wet$values]) #longest run of wets>0
  total_wet_duration <- longest_wet_bins * 2 #each bin = 2hrs
  
  #when avg wets is consistently = 20
  ...
  #find when the longest prd occurs
  end_file <- cumsum(rle_wet$lengths)[which.max(rle_wet$lengths[rle_wet$values])]
  start_file <- end_file - longest_wet +1
  
  datetime_start <- deg_data$datetime_start[start_file]
  datetime_end <- deg_data$datetime_start[end_file]

  
  #return dataframe
  return(data.frame(
    bird_id = bird_id,
    longest_wet = longest_wet / 3600, #divide by 3600 to convert into hours
...
plot_longest_wets_gull_2017 <- ggplot(gull_2017_longest_wet_prds, aes(x=bird_id)) +
  geom_bar(aes(y=longest_wet, fill = "Longest wet"), stat = "identity", position = "dodge") +
  geom_bar(aes(y=longest_very_wet, fill = "Longest very wet"), stat = "identity", position = "dodge") +
  geom_bar(aes(y = longest_entirely_wet, fill = "Longest Entirely Wet"), stat = "identity", position = "dodge") +
   labs(
    title = "Longest Continuous Wet Periods for Gull Island (2017-2018)",
    y = "Time (hours)",
    x = "Bird ID",
    fill = "Condition"
  ) +
...
plotly_gull_2017_longest_wet <- ggplotly(plot_gull_2017_longest_wet) %>%
    layout(
