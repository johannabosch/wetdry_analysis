...
```{r}
example_daily_metrics <- future_map_dfr(example_birds, calc_daily_metrics)
...
plot_daily_prop <- ggplot(example_daily_metrics, aes(x = as.Date(date), y = total_prop_wet, color = bird_id)) +
  geom_line() +
  geom_point() +
  labs(
    title = "Total Proportion of Wet Periods per Day",
...
plot_daily_switch <- ggplot(example_daily_metrics, aes(x = as.Date(date), y = total_switches, color = bird_id)) +
  geom_line() +
  geom_point() +
  labs(
...
```{r}
(plot_daily_prop|plot_daily_switch)
```



```{r}
example_long_daily_metrics <- example_daily_metrics %>%
  pivot_longer(cols=c(total_dry_time, total_wet_time),
               names_to = "metric",
               values_to = "time")

plot_daily_dry_wet <- ggplot(example_long_daily_metrics, aes(x = as.Date(date), y = time, color = bird_id, linetype = metric)) +
  geom_line() +
  geom_point() +
  labs(
    title = "Total Wet-Dry Times per Day",
    y = "Time (seconds)",
    x = "Time (days)",
    color = "Bird ID",
    linetype = "Metric") +
