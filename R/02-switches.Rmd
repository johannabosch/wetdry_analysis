...
To calculate switches between 10min intervals, we need to transform wet values to a binary (wets>0 or wets==0)
Let's add a column to our DEG files that tracks switches, 
...
Finally, we dont want negatives, so we can count absolute values
...
The function below does this for one file
...
The we can apply that function to all our files in our `clipped` directory
```{r}
switch_metrics <- future_map_dfr(chosen_files, calc_switches)
```
