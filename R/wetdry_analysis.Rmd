...
Small note on this file - I accidentally wrote over it on V drive so we have to replace it with the original from backups. Fixed for now by replacing with anothe rfiles metadata info, file is titled `BH594_formatted_04Jul18_135310driftadj.deg` in the origin.dir folder.
...
We need to think about resolution carefully. Right now the files are logging every 10 minutes, which is too high of a resolution. Need to down-sample the dataset from 10min to some other interval that is meaningful to define states to.
...
calc_metrics <- function(file_path) {
  ...
  metrics <- deg_data %>%
    ...
    summarize(
      ...
      #use RLE (run length encoding) to find the longest sequence of wet/dry intervals in 600secs
      longest_dry = max(rle(wets == 0)$lengths) * 10 * 60, #max period 
      longest_wet = max(rle(wets>0)$lengths) *10 * 60, #max period wet
      longest_very_wet = max(rle(wets > 15)$lengths) *10 * 60, #max period wet > 15
      longest_entirely_wet = max(rle(wets == 20)$lengths) *10 * 60) #max period wet == 20
...
#apply calc_metrics using files
metrics_list <- future_map(files, calc_metrics)

#now we have the above metrics for each 2hr period in each file
plan(sequential)
```


Every bird now has the following metrics for every DAY the device was deployed:
...
* **Longest continuous periods of dry or wet states per day:** longest sequence of consecutive 10-minute intervals within a day where the device recorded:
...
```{r}
example_bird <- files[1:2]

example_metrics <- future_ma
